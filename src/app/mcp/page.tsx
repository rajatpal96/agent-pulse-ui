    ...
    <div className="flex-1 flex flex-col">
      ...
      <div className="p-8 max-w-7xl mx-auto w-full space-y-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
          <div className="p-5 rounded-2xl glass-panel border border-emerald-500/20 bg-emerald-500/5">
            ...
            <div className="text-2xl font-bold text-white mt-2 font-mono">
              {data ? formatNumber(data.totalCalls || 0) : 0}
            </div>
            <div className="text-xs text-slate-400 mt-2">
              Across {data?.serversCount || 0} servers & {data?.toolsCount || 0} tools
            </div>
          </div>

          <div className="p-5 rounded-2xl glass-panel border border-teal-500/20 bg-teal-500/5">
            <div className="flex justify-between items-center text-xs text-slate-400 font-semibold uppercase">
            ...
            <div className="text-xs text-emerald-400 mt-2 flex items-center gap-1">
              <ShieldCheck className="w-3.5 h-3.5" /> Performance monitored
            </div>
          </div>
        ...
        <div className="glass-panel rounded-2xl overflow-hidden border border-emerald-500/15 space-y-4 p-6">
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-emerald-500/10 font-mono">
              {(!data?.toolBreakdown || data.toolBreakdown.length === 0) ? (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-emerald-400/60 font-sans">
                    No MCP tool calls logged yet.
                  </td>
                </tr>
              ) : (
                data.toolBreakdown.map((t: any) => (
                  <tr key={`${t.serverName}-${t.toolName}`} className="hover:bg-emerald-950/20 transition-colors">
