  ...
  return (
    <div className="flex-1 flex flex-col">
      ...
      <div className="p-8 max-w-7xl mx-auto w-full space-y-6">
        ...
        {/* Filter Controls */}
        <div className="flex flex-col sm:flex-row gap-4 justify-between items-center bg-[#0c1410] p-4 rounded-2xl border border-emerald-500/15">
          <div className="relative w-full sm:w-80">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-950 border border-emerald-500/20 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-emerald-500 transition-colors"
            />
          </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-medium whitespace-nowrap transition-all ${
                  selectedAgent === agent
                    ...
                    : 'bg-slate-950 text-slate-400 hover:text-emerald-200 border border-emerald-500/15'
                }`}
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              ...
              <tbody className="divide-y divide-emerald-500/10">
                {filtered.length === 0 ? (
                  <tr>
                    <td colSpan={8} className="px-6 py-12 text-center text-emerald-400/60 font-sans">
                      {loading ? 'Loading sessions...' : 'No active or recorded sessions found.'}
                    </td>
                  </tr>
                ) : (
                  filtered.map((s) => (
                    <tr key={s.sessionId} className="hover:bg-emerald-950/20 transition-colors group">
                      <td className="px-6 py-4 font-mono font-semibold text-emerald-400 flex items-center gap-2">
                        <Terminal className="w-3.5 h-3.5 text-slate-400" />
                        {s.sessionId}
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 rounded-full bg-slate-950 border border-emerald-500/20 text-emerald-300 font-medium font-mono text-[11px]">
                          {s.agentName}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-4 text-right">
                        <Link
                          ...
                          className="inline-flex items-center gap-1 px-3 py-1 rounded-lg bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-400 border border-emerald-500/20 font-medium transition-all"
                        >
                          Timeline
